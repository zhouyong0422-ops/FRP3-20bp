...
# 注入深度 CSS 样式，增加权重防止 Streamlit 默认主题干扰，1:1 还原截图 UI
st.markdown("""
<style>
    /* 基础文字与标题 */
    .stApp { font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; color: #333; }
...
# =========================================================================
# 2. 核心引擎 (下划线参数 _progress_bar, _status_text 绕过 Streamlit 缓存校验)
# =========================================================================
...
def run_pipeline_engine(sequences: tuple, _progress_bar=None, _status_text=None):
    ...
    memo_fwd_probes, memo_rev_probes, memo_rev_primers = {}, {}, {}

    def get_all_valid_variants_memo(start_idx: int, is_probe: bool):
        cache_dict = memo_fwd_probes if is_probe else None
        if cache_dict is not None and start_idx in cache_dict: return cache_dict[start_idx]
        valid_list = []
    ...
    all_candidates = []
    total_steps = max(1, seq_len - 70)
    
    for idx, i in enumerate(range(seq_len - 70)):
        # 实时更新进度条与定位坐标
        if _progress_bar and (idx % 15 == 0 or idx == total_steps - 1):
            progress_val = 0.05 + 0.90 * (idx / total_steps)
            _progress_bar.progress(min(0.95, progress_val))
            if _status_text: _status_text.text(f"⏳ [2/2] 全量长度并发寻优中... 当前遍历起始坐标: {i} / {total_steps} bp")
        ...
        for f_obj in f_obj_list[:2]:
            ...
            for r_start in range(min_r_start, max_r_start + 1):
                ...
                for r_obj in r_obj_list[:2]:
                    r_variants_raw, r_variants, r_len = r_obj['rawVariants'], r_obj['variants'], r_obj['length']
                    ...
                    for p_obj in valid_probes[:3]:
                        ...
                        soft_penalty = sum(calc_soft_penalties(f, False) for f in f_variants) + sum(calc_soft_penalties(p, True) for p in p_variants) + sum(calc_soft_penalties(r, False) for r in r_variants)
                        if any(has_3prime_dimer_risk(f, r) for f in f_variants for r in r_variants): soft_penalty += 20.0
                        if abs(primer_max_tm - primer_min_tm) > 2.0: soft_penalty += (abs(primer_max_tm - primer_min_tm) - 2.0) * 5.0
                        ...
                        f_p0, p_p0, r_p0 = float(f_stats['p0']), float(p_stats['p0']), float(r_stats['p0'])
    ...
    if _progress_bar: _progress_bar.progress(0.98)
    ...
    if _status_text: _status_text.text("✅ 寻优全面完成！正在出具 DOE 报告...")
    time.sleep(0.3)
...
# 3. UI 交互与触发
# ==========================================
st.markdown('<div style="background:#f8f9fa; padding:15px; border-radius:8px; border:2px dashed #bdc3c7; margin-bottom:15px; font-size:14px;">📂 <b>V17 核心规范</b>：产物 70–160 bp；探针跟同向引物 &lt;30bp 且跟不同向不限制；解除长度死锁，全量长度优胜劣汰。</div>', unsafe_allow_html=True)
...
if st.session_state.get('has_results', False):
    ...
    # --- 生成导出文件流 ---
    csv_content = "\uFEFF靶区归属,变体角色,综合得分,寡核苷酸类型,序列 (5'->3'),长度 (bp),Tm (°C),GC (%),完全匹配(0),错配1碱基(1),错配2碱基(2),错配3碱基(≥3),预期产物长度 (bp),同向引物间距 (bp),精确起始坐标\n"
    for locus in global_loci_groups:
    ...
    # --- 使用 Streamlit Columns 对齐截图中的头部和下载按钮 ---
    col1, col2 = st.columns([4, 1])
    with col1:
        st.markdown(f'<div class="report-header"><h2>V02 原料齐套：工业级多重引探 DOE 报告</h2><p>出具时间：{st.session_state["report_time"]}</p><p>规范内核：产物 70–160 bp；重罚双套混合；解除长度死锁，所有合法长度竞争入围，优胜劣汰。</p></div>', unsafe_allow_html=True)
    with col2:
        st.write("") # 顶部垂直居中占位
        st.download_button(
            label="📥 导出 Excel (含靶区分组)",
            data=csv_content.encode('utf-8'),
            file_name=f"V02_单向间距竞争优选_DOE清单_{datetime.now().strftime('%Y-%m-%d')}.csv",
            mime="text/csv",
            type="primary",
            use_container_width=True
        )

    # --- 构建高密度无空行 HTML，彻底杜绝 Streamlit Markdown 误解析为“纯文字” ---
    ...
    if not global_loci_groups:
        ...
        for locus in global_loci_groups:
            ...
            for v_idx, cand in enumerate(locus['variants']):
                ...
                # 折叠评分明细
                html_parts.append(f'<details><summary>🔍 查看当前变体柔性评分明细 (全景长度优选版)</summary><div class="details-content">')
                html_parts.append(f'<div class="score-item"><span>基础匹配分 (探针 3 倍权重)</span><span class="score-plus">+{cand["details"]["base"]:.1f}</span></div>')
                html_parts.append(f'<div class="score-item"><span>探针卓越奖励 (完美率 &ge;99%)</span><span class="score-plus">+{cand["details"]["pBonus"]:.1f}</span></div>')
                ...
                html_parts.append(f'<div class="score-item"><span>F/R 混合套数重罚 (-35分/次，极力优选单套)</span><span class="{"" if (cand["details"]["mixF"] + cand["details"]["mixR"]) == 0 else "score-minus"}">{cand["details"]["mixF"] + cand["details"]["mixR"]}</span></div>')
                html_parts.append(f'<div class="score-item" style="border-top:1px solid #ddd;padding-top:4px;font-weight:bold;"><span>柔性规则偏离总扣分 (GC/温差/同向间距过长等)</span><span class="{"" if cand["details"]["softPen"] == 0 else "score-minus"}">{cand["details"]["softPen"]:.1f}</span></div>')
