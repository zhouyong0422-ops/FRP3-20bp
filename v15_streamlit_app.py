...
def is_hard_valid_oligo(seq, is_probe=False):
    if not seq or 'N' in seq or not all(base in 'ATGC' for base in seq):
        return False
    gc = calc_gc(seq)
    if gc < 20 or gc > 80:
        return False
...
def calc_soft_penalties(seq, is_probe):
    penalty = 0
    gc = calc_gc(seq)
    if any(seq.count(base * 4) > 0 for base in 'ATGC'):
        penalty += 10
...
def execute_logic(text):
    ...
    result = {'entropies': entropies, 'sequences': sequences, 'seq_len': seq_len, 'loci_groups': []}
        ...
        for gap1 in range(min_gap, max_gap + 1):
            ...
            for gap2 in range(min_gap, max_gap + 1):
                ...
                for test_len in target_lengths:
                    if r_start + test_len > seq_len: continue
                    ...
                    if all_passed:
                        ...
                        r_len = test_len
                        break
                if not r_found: continue
                
                amplicon_size = r_start + r_len - i
                if amplicon_size < 70 or amplicon_size > 150: continue
                
                # Tm & Penalties (与原JS完全一致)
                ...
                soft_penalty = sum(calc_soft_penalties(f, False) for f in f_variants) + \
                              sum(calc_soft_penalties(p, True) for p in p_variants) + \
                              sum(calc_soft_penalties(r, False) for r in r_variants)
                
                if any(has_3prime_dimer_risk(f, r) for f in f_variants for r in r_variants):
                    soft_penalty += 20
                ...
                all_candidates.append({
                    ...
                    'f_stats': f_stats, 'p_stats': p_stats, 'r_stats': r_stats,
                    'size': amplicon_size, 'start': i, 'score': total_score,
                    'details': {'base': base_score, 'p_bonus': probe_bonus, 'p_penalty': -probe_penalty,
                                'mix_f': mix_f, 'mix_r': mix_r, 'soft_pen': -soft_penalty}
                })
    ...
    for cand in all_candidates:
        ...
        for locus in global_loci_groups:
            if abs(cand['start'] - locus['anchor_start']) <= locus_window:
                ...
                break
        if not found:
            global_loci_groups.append({'locus_id': len(global_loci_groups) + 1, 'anchor_start': cand['start'], 'variants': [cand]})
    ...
    for locus in result['loci_groups']:
        for v_idx, cand in enumerate(locus['variants']):
            ...
            for seq in cand['fwd']:
                ...
                stats = cand['f_stats']
                output.write(f"{locus_name},{role},{score},{typ},{seq},{len(seq)},{calc_tm(seq)},{calc_gc(seq)},{stats['p0']}% ({stats['m0']}/{stats['total']}),{stats['p1']}% ({stats['m1']}/{stats['total']}),{stats['p2']}% ({stats['m2']}/{stats['total']}),{stats['p3']}% ({stats['m3p']}/{stats['total']}),{cand['size']},{cand['start']}\n")
            # Probe & Reverse 类似（省略部分重复代码，逻辑一致）
...
def display_results(result):
    ...
    df_entropy = pd.DataFrame({'Position': list(range(1, len(result['entropies']) + 1)), 'Entropy': result['entropies']})
    fig = px.bar(df_entropy, x='Position', y='Entropy', title='香农熵 (突变强度扫描)',
    ...
    if not result.get('loci_groups'):
        st.error("⚠️ 体系设计失败：在绝对硬底线限制下，该序列库未能找到完整无缺失的组合区域。")
        return
    ...
    for locus in result['loci_groups']:
        ...
        for v_idx, cand in enumerate(locus['variants']):
            ...
            with st.expander(f"{role} - 综合得分: {cand['score']:.1f} 分 | 产物: {cand['size']} bp | 起始: {cand['start']}", expanded=is_primary):
                col1, col2, col3 = st.columns(3)
                with col1:
                    st.markdown("**Forward**")
                    for seq in cand['fwd']:
                        st.code(f"5'- {seq} -3'\nLen: {len(seq)} | Tm: {calc_tm(seq)}°C | GC: {calc_gc(seq)}%", language="text")
                ...
                # 评分明细
                d = cand['details']
                st.metric("基础匹配分", f"+{d['base']:.1f}")
                st.metric("探针奖励/惩罚", f"+{d['p_bonus']:.1f} / {d['p_penalty']:.1f}")
                st.metric("混合套重罚", f"{d['mix_f'] + d['mix_r']}")
                st.metric("柔性扣分", f"{d['soft_pen']:.1f}")
...
st.info("完整复刻版 V15 - 所有规则与原 HTML 版本一致")
